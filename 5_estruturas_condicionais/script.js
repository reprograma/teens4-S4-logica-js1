...
const podeLocomover = prompt(
  "responda com s ou n: tem como se locomover até a loja ou praia?"
);
...
if (taCalor === "n") {
  ...
  alert(
    "se não tem como ir buscar, pede que alguém te traga um sorvete, senão, sinto muito :("
  );
} else if (ehVegan === "s") {
  alert(
    "é difícil achar sorvetes veganos por aí, mas acredito que vai dar certo mana!! o/"
  );
} else {
...
//if ternário - para as curiosas de plantão

totalGramasCafe === 180 && coadorDeCafe === true
  ? console.log("posso fazer o meu café coado")
  : console.log("não posso fazer meu café coado");

// condição ? faça isso : senão passa na condição faça isso. 
// ? - pergunta,      : ou então